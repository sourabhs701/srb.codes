...
In this example, `createFontResizer` returns an object with `increase` and `decrease` methods. These methods have access to the `size` variable and `title` element through closure, even after `createFontResizer` finishes executing. The `size` variable acts as private state that can only be modified through the returned methods.
